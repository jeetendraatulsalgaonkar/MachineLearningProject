...
class MyTestCase(unittest.TestCase):
    ...
    def test_perform_test_train_split_on_data_successfully(self):
        data_ingestion_configuration_entity = DataIngestionConfigurationEntity(
            data_ingestion_directory=os.path.join(DATA_INGESTION_DIRECTORY_NAME),
            data_ingestion_feature_store_directory=os.path.join(DATA_INGESTION_FEATURE_STORE_DIRECTORY),
            data_ingestion_training_file_location=os.path.join(DATA_INGESTION_GENERATED_DIRECTORY, TRAINING_FILE_NAME),
        ...
        data_ingestion: DataIngestion = DataIngestion(data_ingestion_configuration_entity)
        data_frame: DataFrame = data_ingestion.export_data_from_database_into_csv()
        data_ingestion.perform_test_train_split_on_data(data_frame)
        self.assert_is_file(data_ingestion_configuration_entity.data_ingestion_testing_file_location)
        self.assert_is_file(data_ingestion_configuration_entity.data_ingestion_training_file_location)

    def test_perform_test_train_split_on_data_empty_data_frame(self):
        data_ingestion_configuration_entity = DataIngestionConfigurationEntity(
            data_ingestion_directory=os.path.join(DATA_INGESTION_DIRECTORY_NAME),
            data_ingestion_feature_store_directory=os.path.join(DATA_INGESTION_FEATURE_STORE_DIRECTORY),
            data_ingestion_training_file_location=os.path.join(DATA_INGESTION_GENERATED_DIRECTORY, TRAINING_FILE_NAME),
        ...
        self.assertTrue('Error processing the data frame.' in context.exception.error_message)
